...
export async function POST(req: NextRequest, ctx: TaskRouteContext): Promise<NextResponse<SpawnResponse | ApiError>> {
  ...
  try {
    const previous = task.assignments.find((assignment) => assignment.accountId)?.accountId ?? null;
    ...
    if (transcript) {
      patch = { path: transcript, panePid: pane.panePid ?? null, state: "delivered", error: null, at, accountId: account.accountId };
    } else if (pane.panePid) {
      patch = { path: null, panePid: pane.panePid, state: "spawning", error: null, at, accountId: account.accountId };
    } else {
      patch = { path: null, panePid: null, state: "failed", error: "tmux did not return the pane pid", at, accountId: account.accountId };
    }
