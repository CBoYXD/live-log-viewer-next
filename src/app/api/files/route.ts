...
export async function GET(request: Request): Promise<Response> {
  ...
  const requestedRevision = parsedRevision !== undefined && Number.isSafeInteger(parsedRevision) ? parsedRevision : undefined;
  return buildFilesResponse(request, {
    listFilesWithProjectCatalog: async (selectedProject) => {
      const scan = await cachedFileScan(selectedProject, Date.now(), requestedRevision);
