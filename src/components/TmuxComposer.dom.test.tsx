...
test("a lost-response image send clears exactly its snapshot on the delivered replay and rotates the key", async () => {
  const sentKeys: string[] = [];
  ...
  globalThis.fetch = (async (input, init) => {
    ...
    if (sentKeys.length === 2) {
      /* The retry replays the same key; the conflict carries the original
         delivered receipt covering the FIRST attempt's text and image. */
      return {
        ok: false,
        status: 409,
        json: async () => ({
          ...
          error: "idempotency key already delivered",
          receipt: {
            operationId: "op-image-replay",
            idempotencyKey: body.clientMessageId,
            conversationId: "conv-image-loss",
            kind: "send",
            status: "delivered",
  ...
  const file = {
    path: "/codex-image-loss.jsonl",
    root: "codex-sessions",
    name: "codex-image-loss.jsonl",
    project: "viewer",
    title: "Codex image loss",
    engine: "codex",
    kind: "session",
    fmt: "codex",
    parent: null,
  ...
  sessionStorage.setItem("llvDraft:conv-image-loss", "annotate the screenshot");
  const host = document.createElement("div");
  document.body.append(host);
  const root = createRoot(host);
  flushSync(() => root.render(<TmuxComposer file={file} />));
  ...
  /* An image attached while the first attempt's fate is unknown must survive
     the eventual delivered replay of that attempt. */
  ...
  /* Same key both times; the delivered replay clears the sent text and
     exactly the first attempt's image snapshot — the later one stays. */
  expect(sentKeys).toHaveLength(2);
  expect(sentKeys[1]).toBe(sentKeys[0]);
  expect(textarea.value).toBe("");
  expect(previews()).toEqual([laterPreview]);

  /* The consumed key was rotated: the next message sends under a fresh key
     and carries the preserved attachment. */
  flushSync(() => appendComposerDraft("conv-image-loss", "next ask"));
  flushSync(() => form.dispatchEvent(new dom.Event("submit", { bubbles: true, cancelable: true }) as unknown as Event));
  await new Promise((resolve) => setTimeout(resolve, 0));
