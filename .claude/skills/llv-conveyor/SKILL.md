...
3. **Spawn implementer** via `POST 127.0.0.1:8898/api/spawn`. Since #212, non-browser calls are treated as agent-initiated and need `x-llv-spawn-capability` from the caller launch; the orchestrator (no capability yet, see #213) must send same-origin headers instead: `-H 'sec-fetch-site: same-origin' -H 'origin: http://127.0.0.1:8898' -H 'host: 127.0.0.1:8898'`. Keep passing `src`/`role` (`reviews` for reviewers) anyway — the lineage draws the diagram edges. — engine/model per label, `src` = orchestrator transcript (lineage), prompt in English ending with the contract: `FINAL message exactly: REVIEW_READY: <PR url>`, NON-DRAFT PR, tsc+tests must pass, scope fences ("do NOT touch src/lib/{flows,agent,runtime}" for UI lanes).
